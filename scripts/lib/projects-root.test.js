...
// Run directly (`node projects-root.test.js`) or via braynee-self-test.
...
eq('BRAYNEE_PROJECTS_DIR wins',
   getProjectsDir({ BRAYNEE_PROJECTS_DIR: customRoot }),
   path.resolve(customRoot));

eq('BRAYNEE_PROJECTS_DIR marks configured',
   isProjectsDirConfigured({ BRAYNEE_PROJECTS_DIR: customRoot }), true);

eq('BEADS_CODE_DIR honored as legacy override',
   getProjectsDir({ BEADS_CODE_DIR: customRoot }),
   path.resolve(customRoot));

eq('BRAYNEE_PROJECTS_DIR takes precedence over BEADS_CODE_DIR',
   getProjectsDir({ BRAYNEE_PROJECTS_DIR: customRoot, BEADS_CODE_DIR: '/other' }),
   path.resolve(customRoot));

eq('blank/whitespace env falls through to default',
   getProjectsDir({ BRAYNEE_PROJECTS_DIR: '   ' }), path.join(HOME, 'code'));
...
eq('projectRepoPath joins under configured root',
   projectRepoPath('my-app', { BRAYNEE_PROJECTS_DIR: customRoot }),
...
try {
  ...
  eq('join resolves repo under non-~/code root',
     projectRepoPath(repoSlug, { BRAYNEE_PROJECTS_DIR: projectsRoot }),
     repoDir);
  ok('resolved repo dir actually exists in sandbox',
     fs.existsSync(projectRepoPath(repoSlug, { BRAYNEE_PROJECTS_DIR: projectsRoot })));
  ...
  // With BRAYNEE_PROJECTS_DIR pointed at the sandbox root, the folder resolves
  // → NO "folder not found" warning.
  const withEnv = run({ BRAYNEE_PROJECTS_DIR: projectsRoot, BEADS_CODE_DIR: '' });
  const r1 = withEnv.find(r => r.file.endsWith('Sample App.md'));
  ok('prd-audit found the Sample App PRD', !!r1);
  ok('no folder-missing warning when BRAYNEE_PROJECTS_DIR is set correctly',
  ...
  const noEnv = run({ BRAYNEE_PROJECTS_DIR: '', BEADS_CODE_DIR: '' });
  ...
  ok('back-compat warning mentions BRAYNEE_PROJECTS_DIR hint',
     r2 && r2.warnings.some(w => /BRAYNEE_PROJECTS_DIR/.test(w)));
} finally {
